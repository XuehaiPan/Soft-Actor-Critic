...
class Trainer(OriginTrainer):
    ...
    def update(self, batch_size, reward_scale=1.0, auto_entropy=True, target_entropy=-2.0,
        ...
        for state, action, reward, next_state, done in batch:
            state = torch.FloatTensor(state).unsqueeze(dim=1).to(self.device)
            next_state = torch.FloatTensor(next_state).unsqueeze(dim=1).to(self.device)
            action = torch.FloatTensor(action).unsqueeze(dim=1).to(self.device)
            reward = torch.FloatTensor(reward).unsqueeze(dim=1).unsqueeze(dim=2).to(self.device)
            done = torch.FloatTensor(done).unsqueeze(dim=1).unsqueeze(dim=2).to(self.device)
